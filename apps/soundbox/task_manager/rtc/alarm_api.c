...
// 系统时间的
void input_sys_time(void)
{

    sys_current_time.year = temp_year[0] * 1000 + temp_year[1] * 100 + temp_year[2] * 10 + temp_year[3];
    sys_current_time.month =  temp_month[0] * 10 + temp_month[1];
    sys_current_time.day =  temp_day[0] * 10 + temp_day[1];
    sys_current_time.hour = temp_hour[0] * 10 + temp_hour[1];
    sys_current_time.min =  temp_min[0] * 10 + temp_min[1];
    sys_current_time.sec =  temp_sec[0] * 10 + temp_sec[1];
...
void read_sys_current_time(void)
{
    temp_year[0] = sys_current_time.year / 1000;
    temp_year[1] = sys_current_time.year % 1000 / 100 ;
    temp_year[2] = sys_current_time.year % 1000 % 100 / 10;
    temp_year[3] = sys_current_time.year % 1000 % 100 % 10;

    temp_month[0] = sys_current_time.month / 10;
    temp_month[1] = sys_current_time.month % 10;

    temp_day[0] = sys_current_time.day / 10;
    temp_day[1] = sys_current_time.day % 10;

    temp_hour[0] = sys_current_time.hour / 10;
    temp_hour[1] = sys_current_time.hour % 10;

    temp_min[0] = sys_current_time.min / 10;
    temp_min[1] = sys_current_time.min % 10;

    temp_sec[0] = sys_current_time.sec / 10;
    temp_sec[1] = sys_current_time.sec % 10;
...
{
    u8 i;
    User_rtc_load_save(0);  // 获取系统时间
    printf("%d-%d-%d---%d:%d:%d", sys_current_time.year, sys_current_time.month, sys_current_time.day, sys_current_time.hour, sys_current_time.min, sys_current_time.sec);
