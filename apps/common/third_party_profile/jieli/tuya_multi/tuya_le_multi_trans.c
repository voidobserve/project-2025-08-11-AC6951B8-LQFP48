...
#include "tuya_le_multi_common.h"

#define MUTIL_ROLE_IS_SERVER      1
#include "tuya_le_multi_trans.h"

#if (TCFG_BLE_DEMO_SELECT == DEF_BLE_DEMO_TUYA_MULTI&& SUPPORT_MAX_SERVER)
...
static void trans_cbk_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    int mtu;
    u32 tmp;
    u8 status;
...
void server_profile_init(void)
{
    ...
    hci_event_callback_set(&trans_cbk_packet_handler);
    /* ble_l2cap_register_packet_handler(packet_cbk); */
    /* sm_event_packet_handler_register(packet_cbk); */
    le_l2cap_register_packet_handler(&trans_cbk_packet_handler);
...
static int ble_trans_disconnect(void *priv)
{
    if (con_handle) {
...
static const struct ble_server_operation_t mi_ble_operation = {
    ...
    .disconnect = ble_trans_disconnect,
